...
exports.log = function (options) {
  var timestampFn = typeof options.timestamp === 'function'
      ...
      beautifyOptions = options.beautifyOptions || {indent_size:2, wrap_line_length:80, unescape_strings:true},
      output,
      hasHiddenMetaData = false;

  if(options.hideMeta) {
    hasHiddenMetaData = options.meta !== undefined ?
  ...
  if (options.json) {
    ...
    var message = JSON.stringify(output, function (key, value) {
      return value instanceof Buffer
        ? value.toString('base64')
        : value;
    });
    return options.prettyPrint ?
      beautify(message, beautifyOptions)
      : message;
  }
  ...
  output += options.levelPadLength
    ...
    : '';
  var hiddenMetaIndicator = hasHiddenMetaData ? '*' : '';
  output += options.label ?
    ('[' + options.label + hiddenMetaIndicator + '] ')
    : hiddenMetaIndicator;
